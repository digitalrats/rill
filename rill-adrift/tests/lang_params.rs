...
fn param_controls_gain() {
    ...
    let mut prog = compile_with::<f32>("main g = _ * g", &reg, 48_000.0).unwrap();
    let input = vec![1.0f32; 4];
    let mut out = vec![0.0f32; 4];

    let g_idx = prog.param_index("g").expect("param 'g' should exist");
    assert!((prog.param(g_idx).as_f32().unwrap() - 0.0).abs() < 1e-6);
...
fn dynamic_cutoff_changes_filter() {
    ...
    let mut prog_low =
        compile_with::<f32>("main cutoff = _ : lowpass cutoff 0.7", &reg, sr).unwrap();
...
fn lang_node_advertises_and_sets_params() {
    ...
    assert!((g_param.default.as_f32().unwrap() - 0.0).abs() < 1e-6);

    let out_default = render_through(&mut node);
    for &v in &out_default {
        assert!(
            (v - 0.0).abs() < 1e-4,
            "default gain=0: input 2 → output ~0, got {v}"
        );
    }
...
fn ay38910_generates_sound_with_register_write() {
    ...
    let src = r#"main regs = ay38910 1750000.0 regs"#;
    ...
    engine
        .handle()
        .send(rill_core::queues::CommandEnum::SetParameter(
            SetParameter::new(
                PortId::param(NodeId(0), 0),
                ParameterId::new("regs").unwrap(),
                ParamValue::Bytes(regs),
                SignalOrigin::Manual,
            ),
        ));
...
fn ay38910_anchor_map_built_correctly() {
    ...
    let am = engine.param_map();
    assert!(
        am.contains_key("regs"),
        "param_map should contain 'regs' param"
    );
    let idx = am["regs"];
    assert_eq!(idx, 0, "regs should be at param index 0");
    println!("param_map: {:?}", am);
}
    ...
    engine
        ...
        .send(rill_core::queues::CommandEnum::SetParameter(
            SetParameter::new(
                PortId::param(NodeId(0), 0),
                ParameterId::new("regs").unwrap(),
                ParamValue::Bytes(regs.clone()),
                SignalOrigin::Manual,
            ),
        ));

    // First process: drain mailbox, apply pending, then process again
    let mut output = [0.0f32; 64];
    engine.process(None, &mut output).unwrap();
    engine.apply_due_params(64);
    engine.process(None, &mut output).unwrap();
