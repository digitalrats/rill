...
fn test_servo_updates() {
    ...
    println!("Initial signals count: {}", signal_sender.get_signals_count());
    println!("Manager has signal_sender: {:?}", manager.context().signal_sender.is_some());
    ...
    for i in 1..=3 {
        ...
        println!("After update - signals count: {}", signal_sender.get_signals_count());
...
fn test_servo_range() {
    ...
    for &value in &signals {
        assert!(value >= 0.2 && value <= 0.8, "Value {} out of range [0.2, 0.8]", value);
    }
}
...
fn test_servo_persistence() {
    ...
    println!("test_servo_persistence - collected {} unique values: {:?}", values.len(), values);
    assert!(values.len() >= 2, "Should have at least 2 different values, got {}", values.len());
}
...
fn test_signal_sender_integration() {
    ...
    assert!(signals.len() >= 2, "Expected at least 2 signals, got {}", signals.len());
