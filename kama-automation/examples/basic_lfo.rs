...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let lfo_automaton = FunctionAutomaton::lfo_with_waveform(
        0.7, 0.2, 0.6, LfoWaveform::Triangle,
        "mixer_channel_2", "pan"
    );
    ...
    for i in 0..20 {
        ...
        let volume = signal_sender.get_signals_for_param("mixer_channel_1", "volume")
            .last().copied().unwrap_or(0.5);
        let pan = signal_sender.get_signals_for_param("mixer_channel_2", "pan")
