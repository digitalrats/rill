...
fn main() {
    ...
    for i in 0..buffer_size {
        test_buffer.as_mut_slice()[i] = (2.0 * std::f32::consts::PI * 440.0 * i as f32 / sample_rate).sin() * 0.5;
    }
    ...
    for block in 0..num_blocks {
        ...
        let max_left = output_left.as_slice().iter().map(|&x| x.abs()).fold(0.0f32, |a, b| a.max(b));
        let max_right = output_right.as_slice().iter().map(|&x| x.abs()).fold(0.0f32, |a, b| a.max(b));
