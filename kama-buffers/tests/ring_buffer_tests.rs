...
fn test_ring_buffer_lookahead_after_overwrite() {
    ...
    buffer.write(&[1.0, 2.0, 3.0, 4.0]);
    println!("After first write: write_pos={}, filled={}", buffer.write_pos(), buffer.is_filled());
    
    buffer.write(&[5.0, 6.0]);
    println!("After second write: write_pos={}, filled={}", buffer.write_pos(), buffer.is_filled());
...
fn test_ring_buffer_lookahead_stress() {
    ...
    for lookahead in 0..32 {
        ...
        assert_eq!(view.read_lookahead(lookahead, 0), expected,
                   "Mismatch at lookahead {}", lookahead);
    }
}
...
fn test_ring_buffer_lookahead_with_offset_stress() {
    ...
    for lookahead in 0..4 {
        for offset in 0..4 {
            ...
            assert_eq!(view.read_lookahead(lookahead, offset), expected,
                       "Mismatch at lookahead={}, offset={}", lookahead, offset);
        }
    }
}
