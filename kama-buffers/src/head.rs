...
impl BufferHead {
    ...
    pub fn read_sample(&mut self, view: &BufferView) -> f32 {
        ...
        let sample = match self.read_mode {
            ReadMode::Simple | ReadMode::Loop => self.read_linear(view, buffer_size),
            ReadMode::PingPong => self.read_pingpong(view, buffer_size),
            ReadMode::Granular {
                grain_size,
                spacing,
                randomization,
    ...
    fn read_linear(&self, view: &BufferView, buffer_size: f32) -> f32 {
        match self.state.direction {
            Direction::Forward => view.get_interpolated(self.state.position),
            Direction::Reverse => {
