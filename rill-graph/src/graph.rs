...
use rill_core::traits::ConnectionError;
use rill_core::traits::{
    ActivePort, AudioNode, NodeVariant, ProcessContext, Processable, Processor, Sink, Source,
};
use rill_core::traits::{ProcessError as GraphError, ProcessResult as GraphResult};
...
impl<T: AudioNum, const BUF_SIZE: usize> AudioGraph<T, BUF_SIZE> {
    ...
    pub fn process_block(&mut self) -> GraphResult<()> {
        ...
        self.stats.avg_process_time_ns =
            self.stats.avg_process_time_ns * 0.95 + elapsed_ns as f64 * 0.05;
    ...
    fn process_node(&mut self, node_id: NodeId) -> GraphResult<()> {
        ...
        let mut feedback_output_refs: Vec<&mut [T; BUF_SIZE]> =
            feedback_outputs.iter_mut().collect();
...
mod tests {
    ...
    fn test_builder_add_nodes() {
        // Temporarily disabled due to missing rill_core_dsp dependency
    }
}
