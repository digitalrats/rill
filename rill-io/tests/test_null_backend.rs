use rill_io::{
    backends::NullBackend, processor::GainProcessor, AudioBackend, AudioConfig, AudioEngine,
    AudioProcessor,
};
...
fn test_audio_engine_with_null_backend() {
    ...
    assert_eq!(engine.state(), rill_io::EngineState::Stopped);

    engine.start().unwrap();
    assert_eq!(engine.state(), rill_io::EngineState::Running);
...
fn test_audio_engine_update_processor() {
    ...
    engine
        .update_processor(|p: &mut GainProcessor| {
            p.set_gain(2.0);
        })
        .unwrap();
