...
pub use kama_dsp_common::filter::{Filter, FilterType, FilterFactory};
pub use biquad::{BiquadFilter, BiquadFactory};
...
mod tests {
    ...
    fn test_biquad_lowpass() {
        ...
        for i in 0..1000 {
            ...
            if i > 900 {  // Последние 100 семплов
                steady_state += output;
            }
        }
        ...
        assert!(steady_state > 0.95 && steady_state < 1.05, 
                "DC steady state should be near 1.0, got {}", steady_state);
        ...
        assert!(max_output < 0.5, "High frequency should be attenuated, got {}", max_output);
    }
