...
impl<T: AudioNum, const MAX_DELAY: usize> Delay<T, MAX_DELAY> {
    ...
    fn update_delay_samples(&mut self) {
        self.delay_samples = crate::math::seconds_to_samples(
...
impl<T: AudioNum, const MAX_DELAY: usize> Algorithm<T> for Delay<T, MAX_DELAY> {
    ...
    fn process_sample(&mut self, input: T) -> T {
        ...
        let write_signal = if self.params.invert_feedback {
            input.sub(delayed.mul(feedback))
        } else {
            input.add(delayed.mul(feedback))
        };
    ...
    MultiTapDelay<T, MAX_DELAY, MAX_TAPS> 
{
    ...
    fn update_tap_samples(&mut self, index: usize) {
        self.tap_samples[index] = crate::math::seconds_to_samples(
            self.taps[index].delay_time,
            self.sample_rate
        );
        debug_assert!(
            self.tap_samples[index] < MAX_DELAY,
            "Tap delay too long"
        );
    }
    ...
    for MultiTapDelay<T, MAX_DELAY, MAX_TAPS>
{
    ...
    fn process_sample(&mut self, input: T) -> T {
        ...
        let _ = self.delay_line.write(input);
    ...
    DiffusionDelay<T, STAGES, MAX_DELAY> 
{
    ...
    pub fn process_diffusion(&mut self, input: T) -> T {
        ...
        for i in 0..STAGES {
            ...
            let y = x.mul(g.neg()).add(delayed).add(self.delays[i].read().mul(g));
    ...
    for DiffusionDelay<T, STAGES, MAX_DELAY>
{
    fn init(&mut self, sample_rate: f32) {
        ...
        for i in 0..STAGES {
            let delay_samples = crate::math::seconds_to_samples(
                self.delay_times[i],
                sample_rate
            );
...
impl<T: AudioNum, const MAX_DELAY: usize> Algorithm<T> for ModulatedDelay<T, MAX_DELAY> {
    ...
    fn process_sample(&mut self, input: T) -> T {
        ...
        // Обратная связь
        let feedback = T::from_f32(self.params.feedback);
        let write_signal = input.add(delayed.mul(feedback));
        let _ = self.delay_line.write(write_signal);
        ...
        dry.mul(one_minus_mix).add(wet.mul(mix))
    }
...
mod tests {
    ...
    fn test_delay_basic() {
        ...
        assert_eq!(delay.process_sample(1.0), 0.5);
    ...
    fn test_delay_feedback() {
        ...
        let out2 = delay.process_sample(0.0);
