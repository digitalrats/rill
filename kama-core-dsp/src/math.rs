...
pub fn lagrange_interpolate<T: AudioNum>(y: &[T; 4], x: T) -> T {
    ...
    let term0 = y[0].mul(
        (x.sub(x1)).mul(x.sub(x2)).mul(x.sub(x3))
        .div((x0.sub(x1)).mul(x0.sub(x2)).mul(x0.sub(x3)))
    );
    
    let term1 = y[1].mul(
        (x.sub(x0)).mul(x.sub(x2)).mul(x.sub(x3))
        .div((x1.sub(x0)).mul(x1.sub(x2)).mul(x1.sub(x3)))
    );
    
    let term2 = y[2].mul(
        (x.sub(x0)).mul(x.sub(x1)).mul(x.sub(x3))
        .div((x2.sub(x0)).mul(x2.sub(x1)).mul(x2.sub(x3)))
    );
    
    let term3 = y[3].mul(
        (x.sub(x0)).mul(x.sub(x1)).mul(x.sub(x2))
...
impl<T: AudioNum> Smoother<T> {
    ...
    pub fn next(&mut self) -> T {
        self.current = self.current.add(
            self.target.sub(self.current).mul(self.coeff)
        );
        self.current
    }
...
// В файле kama-core-dsp/src/math.rs
...
mod tests {
    ...
    fn test_midi_conversion() {
        ...
        assert!(freq_low > 0.0 && freq_low < 100.0, "MIDI 0 should be low frequency, got {}", freq_low);
        
        let freq_high: f32 = midi_to_freq(127);
        println!("MIDI 127 -> frequency: {:.6} Hz", freq_high);
        assert!(freq_high > 10000.0, "MIDI 127 should be high frequency, got {}", freq_high);
    }
    ...
    fn test_fast_tanh() {
        ...
        for &x in &test_values {
            ...
            println!("x = {:4.1}: exact = {:8.6}, fast = {:8.6}, diff = {:8.6}", 
                     x, exact, fast, diff);
    ...
    fn test_lerp() {
        ...
        for (a, b, t, expected) in test_cases {
            ...
            println!("lerp({}, {}, {}) = {}, expected {}", a, b, t, result, expected);
            assert!(
    ...
    fn test_seconds_to_samples() {
        ...
        for (seconds, expected) in test_cases {
            ...
            assert_eq!(samples, expected, "{} seconds should be {} samples", seconds, expected);
            ...
            assert!(
                ...
                "Round trip failed: {} -> {} -> {}",
                seconds, samples, back_to_seconds
            );
        }
    }
    ...
    fn test_sine_phase() {
        ...
        for &phase in &test_phases {
            ...
            if (phase - 0.0).abs() < EPSILON {
                assert!((val - 0.0).abs() < EPSILON, "sin(0) should be 0, got {}", val);
            }
            if (phase - 0.25).abs() < EPSILON {
                assert!((val - 1.0).abs() < EPSILON, "sin(π/2) should be 1, got {}", val);
            }
            if (phase - 0.5).abs() < EPSILON {
                assert!((val - 0.0).abs() < EPSILON, "sin(π) should be 0, got {}", val);
            }
        }
    }
...
fn test_triangle_phase() {
    ...
    for &phase in &test_phases {
        ...
        if (phase - 0.0).abs() < 1e-6 {
            assert!((val - -1.0).abs() < 1e-4, "triangle(0) should be -1, got {}", val);
        } else if (phase - 0.25).abs() < 1e-6 {
            assert!((val - 0.0).abs() < 1e-4, "triangle(0.25) should be 0, got {}", val);
        } else if (phase - 0.5).abs() < 1e-6 {
            assert!((val - 1.0).abs() < 1e-4, "triangle(0.5) should be 1, got {}", val);
        } else if (phase - 0.75).abs() < 1e-6 {
            assert!((val - 0.0).abs() < 1e-4, "triangle(0.75) should be 0, got {}", val);
        } else if (phase - 1.0).abs() < 1e-6 {
            assert!((val - -1.0).abs() < 1e-4, "triangle(1.0) should be -1, got {}", val);
        }
    }
}
}