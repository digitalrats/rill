...
│                      Инфраструктура                           │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐             │
│  │rill-osc │  │rill-graph  │  │rill-patchbay│             │
...
- **Для signal processing**: фильтры, генераторы, FFT для любых временных рядов
- **Для всех**: domain-agnostic ядро с нулевыми затратами на абстракцию
...
Только `rill-io` привязан к аудио-железу (ALSA, CPAL, JACK, PipeWire).
Остальные крейты — **domain-agnostic инструменты**, которые просто хорошо решают свою задачу в аудио-контексте.
...
graph TD
    ...
    %% Планируемые
    SERVER[rill-osc<br/>(планируется)]
    CORE -.-> SERVER
