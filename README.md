...
Проект находится в стадии активной разработки. Все 14 крейтов workspace активны. Аналоговое моделирование представлено тремя крейтами: `rill-core-wdf` (WDF ядро), `rill-analog-filters` (WdfMoogLadder) и `rill-analog-effects` (операционники, ленточные деки). Крейт `rill-server` находится в разработке, а `rill-tests` запланирован к реализации. Актуальное состояние архитектуры и дорожная карта доступны в [architecture.md](architecture.md).
...
- 🧪 **rill-tests** — создание интеграционных тестов
