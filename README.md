...
│  │(протокол) │  │(DAG граф) │ │(автоматизация)│ │sampler│  │
│  └────────────┘  └────────────┘  └────────────┘  └──────┘  │
├─────────────────────────────────────────────────────────────┤
│                      Обработка сигналов                       │
│  ┌─────────────────────────────────────────────────────┐    │
...
- **Для signal processing**: фильтры, генераторы, интерполяция для любых временных рядов
- **Для embedded**: lock-free очереди без ОС, векторная математика на целых числах
- **Для IoT/CAN-bus**: TimeSeriesReader для неравномерных данных, мультиканальный вывод
- **Для аудио**: готовые Source/Processor/Sink узлы графа, WAV loading, OSC


## ✨ Особенности

- **Domain-agnostic ядро** — `Scalar`, `Vector<T>`, lock-free очереди — работают вне аудио
...
- **Тестируемость** — 300+ тестов, всё проверено
- **Расширяемость** — легко добавить свой эффект или бэкенд

## 🎯 За пределами аудио

Rill не про звук. Базовая инфраструктура применима в любых областях:

| Компонент | IoT | Embedded | Robotics | Signal Processing | Control |
|-----------|-----|----------|----------|-------------------|---------|
| `Scalar` + `Vector<T>` | ✅ sensor fusion | ✅ fixed-point | ✅ kinematics | ✅ windowing | ✅ |
| `SpscQueue` / `RingQueue` | ✅ ISR→main | ✅ no_std, no OS | ✅ RT comms | ✅ pipeline | ✅ |
| `GraphBuilder` (DAG) | ✅ data pipeline | — | ✅ task graph | ✅ flow | ✅ state machine |
| `PatchbayControl` | ✅ automation | — | ✅ sequence | ✅ modulation | ✅ SCADA |
| `WdfElement<T: Scalar>` | — | ✅ circuit sim | — | — | — |
| `TimeSeriesReader<T>` | ✅ CAN logs | ✅ sensor data | ✅ telemetry | ✅ resampling | ✅ |
| `Interpolate` trait | ✅ uneven data | ✅ lookup tables | ✅ motion planning | ✅ curve fitting | ✅ |

Крейты `rill-io` и `rill-osc` привязаны к аудио-железу (ALSA, CPAL, JACK, PipeWire)
и OSC-протоколу. Остальные крейты — **domain-agnostic инструменты**.

## 🚀 Быстрый старт

### Не-аудио пример: интерполяция временных рядов

Добавьте `rill-core` (чистая математика, без аудио-зависимостей):
...
let sensor_data = vec![0.0, 1.5, 3.0, 2.0, 0.0];

// Линейная интерполяция в дробной позиции
let v = sensor_data.interpolate_linear(2.3);
assert!((v - 2.7).abs() < 1e-6);

// Nearest-neighbour — без арифметики, работает для любых Scalar
let nearest = sensor_data.interpolate_nearest(2.3);
// Cubic Hermite — гладкие кривые
let smooth = sensor_data.interpolate_cubic(2.3);
```

### Аудио пример: DSP pipeline (синус → задержка)

Добавьте `rill-core-dsp`:
...
- **`Scalar`** — базовый: `+`, `-`, `*`, `/`, `min`, `max`, `clamp`, `abs`. Реализован для `f32`, `f64`, `i8`, `i16`, `i32`, `i64`.
- **`Transcendental`** — расширяет `Scalar`: `sin`, `cos`, `sqrt`, `exp`, `ln`, `PI`. Только `f32`, `f64`.
- **`Vector<T: Scalar, N>`** — векторные операции (SIMD-ready) для любого `Scalar`.

```rust
use rill_core::math::Scalar;

// Работает с любым Scalar — хоть f32, хоть i32
fn scale<T: Scalar>(v: ScalarVector4<T>) -> ScalarVector4<T> {
    v * T::from_f32(0.5)  // только при T: Transcendental
}
```
...
- [План разработки](docs/plans/two_thread_architecture.md) — архитектура двухпоточности
...
- 🚦 **Развитие rill-router** — добавление матричной маршрутизации, расширение модуля `mixer`, интеграция с графом
...
## 🔗 Граф сигналов: математически строгая модель обработки

> **⚠️ Целевой API.** Примеры в этом разделе описывают проектируемый интерфейс `rill-graph`. Текущая реализация использует `GraphBuilder` для конструирования графа. Актуальный API см. в [`rill-graph/src/graph.rs`](rill-graph/src/graph.rs).
...
|-----------|----------|---------|
| **`Signal`** | Сигнальные данные (блоки фиксированного размера) | Данные с источника, выход процессора |
...
impl<T: Transcendental, const BUF_SIZE: usize> Sink<T, BUF_SIZE> for AlsaSink<T, BUF_SIZE> {
    fn consume(
        &mut self,
        clock: &ClockTick,
...
```rust
struct ResonantFilter<T: Transcendental, const BUF_SIZE: usize> {
    // feedback-порты хранят состояние
    feedback: [T; 2],  // y[n-1] и y[n-2]
}

impl<T: Transcendental, const BUF_SIZE: usize> Processor<T, BUF_SIZE> for ResonantFilter<T, BUF_SIZE> {
    fn process(
        &mut self,
        clock: &ClockTick,
        audio_inputs: &[&[T; BUF_SIZE]],
...
**Rill Patchbay** — это не просто система управления. Это **мир**, в котором живут **автоматы** — загадочные существа, которые чувствуют окружающую среду и влияют на неё. Они общаются на языке сигналов, через сенсоры и серво воздействуют на SignalGraph.
...
Мир автоматов и мир сигналов существуют параллельно. Они связаны **неблокирующими очередями**:
...
## 🔗 rill-graph — иммутабельный граф сигналов (Static DAG)

Библиотека для построения и выполнения иммутабельных графов сигналов со статической топологией DAG:
...
- **Zero-copy routing** — для 1:1 и fan-out соединений; копирование только при fan-in и feedback
- **Поддержка Feedback** — отложенная обратная связь через `feedback_buffer`
- **Safe Rust** — `unsafe` только для deref `upstream_buffer` в rill-core
...
- **Zero-copy routing** — для 1:1 и fan-out (нет ветвления), копия только при fan-in и feedback
