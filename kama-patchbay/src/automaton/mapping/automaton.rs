                        ...
                        if let (Some(port), Some(param)) = (&rule.target_port, &rule.target_parameter) {
                            let cmd = kama_core::queues::SetParameter::new(
                                *port,
                                param.clone(),
                                output_val,
                                kama_core::queues::SignalSource::Automaton(self.name.clone()),
                            );
                            let _ = context.command_tx.send(kama_core::queues::CommandEnum::SetParameter(cmd));
                        }
                        
                        // Отправляем телеметрию
                        let _ = context.telemetry_tx.send(
                            kama_core::queues::Telemetry::event(
                                self.name.clone(),
                                "mapping",
