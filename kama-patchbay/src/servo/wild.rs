...
use kama_core::queues::MicroControlObserver;
...
pub struct SafeServo<A: Automaton> {
    ...
    state: A::State,
    target: ParameterTarget,
    cmd_queue: crossbeam_channel::Sender<kama_core::queues::Command>,
    last_value: f32,
}
...
impl<A: Automaton> SafeServo<A> {
    ...
    pub fn update(&mut self, time: WorldTime) {
        ...
        let cmd = kama_core::queues::SetParameter::new(
            self.target.port,
        ...
        let _ = self.cmd_queue.send(kama_core::queues::CommandEnum::SetParameter(cmd));
    }
}