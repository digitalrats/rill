...
use kama_eq::{ParametricEq, GraphicEq, BandType};
use kama_digital_filters::{BiquadFilter, BiquadFactory, FilterType};
use kama_core_traits::{AudioNode, ParamValue};
use kama_core_traits::Clock;  // для advance
use kama_automation::{AutomationManager, TestSignalSender, Servo, ParameterMapping};
...
fn test_parametric_eq_band_boost() {
    ...
    for &freq in &test_freqs {
        ...
        println!("Frequency {}: input_rms = {:.6}, output_rms = {:.6}", 
                 freq, input_rms, output_rms);
        
        let gain = if input_rms > 0.0 { output_rms / input_rms } else { 1.0 };
        
        results.push((freq, gain));
        
        // Peak gain for verification
        let max_output = output.iter().skip(1000).map(|&x| x.abs()).fold(0.0f32, |a, b| a.max(b));
        let max_input = signal.iter().skip(1000).map(|&x| x.abs()).fold(0.0f32, |a, b| a.max(b));
        let peak_gain = if max_input > 0.0 { max_output / max_input } else { 1.0 };
        
        println!("Frequency: {:6.0} Hz | Gain: {:.3} ({:.1} dB) | Peak gain: {:.3} | input_rms: {:.3}",
                 freq, gain, 20.0 * gain.log10(), peak_gain, input_rms);
    }
    
    // Verify boost at 1kHz
    let boost_at_1k = results.iter().find(|(f, _)| (*f - 1000.0).abs() < 1.0).unwrap().1;
    println!("Boost at 1kHz: {:.3} ({:.1} dB)", boost_at_1k, 20.0 * boost_at_1k.log10());
    
    assert!((boost_at_1k - 2.0).abs() < 0.8,
            "Boost at 1kHz should be near 2.0 (6dB), got {:.3}", boost_at_1k);
}
...
fn test_parametric_eq_band_cut() {
    ...
    for &freq in &test_freqs {
        ...
        let input_rms = (signal.iter().skip(1000).map(|&x| x * x).sum::<f32>() / count as f32).sqrt();
        let gain = rms / input_rms;
        
        results.push((freq, gain));
        println!("Frequency: {:6.0} Hz | Gain: {:.3} ({:.1} dB)", 
                 freq, gain, 20.0 * gain.log10());
    }
    
    // Verify cut at 1kHz
    let cut_at_1k = results.iter().find(|(f, _)| (*f - 1000.0).abs() < 1.0).unwrap().1;
    println!("Cut at 1kHz: {:.3} ({:.1} dB)", cut_at_1k, 20.0 * cut_at_1k.log10());
    
    // Should be close to 0.5 (-6dB)
    assert!((cut_at_1k - 0.5).abs() < 0.2,
            "Cut at 1kHz should be near 0.5 (-6dB), got {:.3}", cut_at_1k);
}
...
fn test_parametric_eq_parameters() {
    ...
    eq.set_param("band_0_freq", ParamValue::Float(200.0)).unwrap();
    ...
    eq.set_param("band_0_enabled", ParamValue::Bool(false)).unwrap();
...
fn test_eq_automation_integration() {
    ...
    assert!(!output_signals.is_empty(), "No output gain automation signals");
    
    // Verify that values are within expected ranges
    for &value in &freq_signals {
        assert!(value >= 300.0 && value <= 700.0, 
                "Frequency out of range: {}", value);
    }
    
    for &value in &gain_signals {
        assert!(value >= -3.0 && value <= 3.0, 
                "Gain out of range: {}", value);
    }
...
fn test_eq_boundary_conditions() {
    ...
    eq.set_param("band_0_freq", ParamValue::Float(10.0)).unwrap(); // below min
    match eq.get_param("band_0_freq") {
        Some(ParamValue::Float(v)) => assert!((v - 20.0).abs() < 0.1), // should clamp to 20
        _ => panic!("band_0_freq should be float"),
    }
    
    eq.set_param("band_0_freq", ParamValue::Float(30000.0)).unwrap(); // above max
    match eq.get_param("band_0_freq") {
    ...
    eq.set_param("band_0_gain", ParamValue::Float(30.0)).unwrap(); // above max
    match eq.get_param("band_0_gain") {
        Some(ParamValue::Float(v)) => assert!((v - 24.0).abs() < 0.1), // should clamp to 24
        _ => panic!("band_0_gain should be float"),
    }
    
    eq.set_param("band_0_gain", ParamValue::Float(-30.0)).unwrap(); // below min
    match eq.get_param("band_0_gain") {
        Some(ParamValue::Float(v)) => assert!((v + 24.0).abs() < 0.1), // should clamp to -24
        _ => panic!("band_0_gain should be float"),
    }
