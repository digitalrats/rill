use kama_digital_effects::{Delay, Distortion, Limiter, DistortionType};
...
fn test_delay_parameters() {
    ...
    delay.set_param("delay_time", ParamValue::Float(0.5)).unwrap();
...
fn test_distortion_parameters() {
    ...
    dist.set_param("output_gain", ParamValue::Float(1.2)).unwrap();
    dist.set_param("type", ParamValue::Choice("hard_clip".to_string())).unwrap();
...
fn test_distortion_types() {
    ...
    for &dist_type in &[DistortionType::HardClip, DistortionType::SoftClip, 
        ...
        for &input in &test_inputs {
            ...
            assert!(output >= -2.0 && output <= 2.0, 
                    "Output out of range: {}", output);
        }
    }
}
...
fn test_limiter_basic() {
    ...
    println!("Low signal - input: {}, output: {}", test_input, test_output);
    
    // Должен пропускать без изменений (gain ~1.0)
    assert!((test_output - test_input).abs() < 0.1, 
            "Low signal should pass through, got {}", test_output);
    ...
    for i in 0..2000 {
        ...
        outputs.push(out);
        if i % 200 == 0 {
            println!("Sample {}: output = {:.3}, current_gain = {:.3}", 
                     i, out, limiter.current_gain());
        }
    }
    ...
    assert!(avg > 0.0, "Average output should be positive, got {:.3}", avg);
    assert!(max_val < high_input, "Max output should be less than input");
    assert!(max_val > 0.0, "Max output should be positive");
    assert!(max_val < threshold_linear * 1.2, 
            "Max output should be near threshold, got {:.3}", max_val);
...
fn test_limiter_envelope() {
    ...
    println!("\nGenerating signal with peak at samples {}-{}", peak_start, peak_end);
    
    for i in 0..total_samples {
        let input = if i >= peak_start && i < peak_end { 2.0 } else { 0.1 };
    ...
    let max_idx = outputs.iter().enumerate()
    ...
    assert!(max_output > 0.8 && max_output < 1.2,
            "Peak should be around {:.3}, got {:.3}", expected_max, max_output);
    ...
    if after_peak < outputs.len() {
        ...
        assert!((recovery - 0.1).abs() < 0.15,
                "After peak, output should return near 0.1, got {:.3}", recovery);
    }
...
fn test_limiter_parameters() {
    ...
    assert_eq!(limiter.get_param("threshold"), Some(ParamValue::Float(-3.0)));
    ...
    assert_eq!(limiter.get_param("output_gain"), Some(ParamValue::Float(1.5)));
    
    limiter.set_param("threshold", ParamValue::Float(-10.0)).unwrap();
    limiter.set_param("attack", ParamValue::Float(0.02)).unwrap();
    limiter.set_param("release", ParamValue::Float(0.3)).unwrap();
    limiter.set_param("output_gain", ParamValue::Float(0.8)).unwrap();
    
    assert_eq!(limiter.get_param("threshold"), Some(ParamValue::Float(-10.0)));
    ...
    assert_eq!(limiter.get_param("output_gain"), Some(ParamValue::Float(0.8)));
...
fn test_limiter_reset() {
    ...
    for i in 0..200 { // Увеличили до 200 семплов
        let out = limiter.process_sample(1.5);
        if i == 0 || i == 100 || i == 199 {
            println!("  Step {}: gain={:.3}, out={:.3}", i, limiter.current_gain(), out);
        }
    }
    ...
    assert!(gain_before < 0.8, "Gain should be reduced (<0.8), got {:.3}", gain_before);
    ...
    assert!(gain_after < 0.8, "Gain should be reduced again (<0.8), got {:.3}", gain_after);
    
    // Проверяем, что выход ограничен
    assert!(max_out < 1.0, "Output should be limited (<1.0), got {:.3}", max_out);
