...
use kama_lofi::{
    LofiProcessor, LofiConfig, ClassicSystem,
    emulators::{NesEmulator, Ay38910Emulator, AkaiS900Emulator},
    AudioNode, dsp
};
...
fn test_lofi_processor_parameters() {
    ...
    // Test get_param
    let params = ["bit_depth", "dry_wet", "output_gain", "enable_bitcrush", "num_heads"];
    for &name in &params {
    ...
    processor.set_param("dry_wet", ParamValue::Float(0.7)).unwrap();
    assert_eq!(processor.get_param("dry_wet"), Some(ParamValue::Float(0.7)));
    
    processor.set_param("output_gain", ParamValue::Float(2.0)).unwrap();
    assert_eq!(processor.get_param("output_gain"), Some(ParamValue::Float(2.0)));
    
    processor.set_param("enable_bitcrush", ParamValue::Bool(false)).unwrap();
    assert_eq!(processor.get_param("enable_bitcrush"), Some(ParamValue::Bool(false)));
...
fn test_lofi_processor_reset() {
    ...
    assert_eq!(processor.get_param("processed_samples"), Some(ParamValue::Int(0)));
...
fn test_lofi_processor_process_basic() {
    ...
    processor.set_param("enable_bitcrush", ParamValue::Bool(false)).unwrap();
    processor.set_param("enable_sr_reduction", ParamValue::Bool(false)).unwrap();
    processor.set_param("enable_noise", ParamValue::Bool(false)).unwrap();
    processor.set_param("output_gain", ParamValue::Float(0.8)).unwrap();
...
fn test_lofi_processor_bitcrush() {
    ...
    processor.set_param("enable_bitcrush", ParamValue::Bool(true)).unwrap();
    processor.set_param("enable_sr_reduction", ParamValue::Bool(false)).unwrap();
    processor.set_param("enable_noise", ParamValue::Bool(false)).unwrap();
    processor.set_param("dry_wet", ParamValue::Float(1.0)).unwrap();  // 100% wet
...
fn test_lofi_processor_sample_rate_reduction() {
    ...
    processor.set_param("enable_bitcrush", ParamValue::Bool(false)).unwrap();
    processor.set_param("enable_sr_reduction", ParamValue::Bool(true)).unwrap();
    processor.set_param("enable_noise", ParamValue::Bool(false)).unwrap();
    ...
    assert!(repeats > 100, "Too few repeats ({}) - sample rate reduction may not be working", repeats);
...
fn test_lofi_processor_noise_addition() {
    ...
    processor.set_param("enable_bitcrush", ParamValue::Bool(false)).unwrap();
    processor.set_param("enable_sr_reduction", ParamValue::Bool(false)).unwrap();
    processor.set_param("enable_noise", ParamValue::Bool(true)).unwrap();
...
fn test_nes_emulator() {
    ...
    assert!(first_few.iter().any(|&x| x != first_few[0]), 
            "Signal should vary");
}
...
fn test_dsp_quantization() {
    ...
    for &bits in &[8, 12, 16] {
        let quantized: Vec<f32> = test_values.iter()
        ...
        if bits == 8 {
            ...
            for i in 0..test_values.len() {
                if (test_values[i] - 0.5).abs() > 0.01 {  // Пропускаем 0.5
            ...
            if !changed {
                println!("    12-bit changes are very small (less than 0.0001) - this is acceptable");
            } else {
