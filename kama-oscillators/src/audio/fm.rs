...
mod tests {
    ...
    fn test_fm_osc_parameters() {
        ...
        osc.set_param("carrier_freq", ParamValue::Float(880.0))
            .unwrap();
        assert_eq!(osc.carrier_freq, 880.0);

        osc.set_param("modulator_ratio", ParamValue::Float(2.0))
            .unwrap();
        assert_eq!(osc.modulator_ratio, 2.0);

        osc.set_param("modulation_index", ParamValue::Float(1.5))
            .unwrap();
