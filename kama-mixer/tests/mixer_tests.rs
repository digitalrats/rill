use kama_mixer::{MixerNode, ChannelConfig, ChannelMode, SendConfig, SendType};
...
fn test_mixer_basic_processing() {
    ...
    for i in 0..100 {
        assert!(approx_eq!(f32, output_left[i], expected, epsilon = 0.001),
                "left[{}]: {} vs {}", i, output_left[i], expected);
        assert!(approx_eq!(f32, output_right[i], expected, epsilon = 0.001),
                "right[{}]: {} vs {}", i, output_right[i], expected);
    }
}
...
fn test_mixer_pan() {
    ...
    assert!(output_left[0] != output_right[0], "Left and right should be different with pan");
}
...
fn test_mixer_sends() {
    ...
    for i in start..100 {
        assert!(approx_eq!(f32, output_left[i], 0.8, epsilon = 0.01),
                "left[{}]: {} vs 0.8", i, output_left[i]);
        assert!(approx_eq!(f32, output_right[i], 0.8, epsilon = 0.01),
                "right[{}]: {} vs 0.8", i, output_right[i]);
        
        // Post-fader send: signal * volume * level = 1.0 * 0.8 * 0.5 = 0.4
        assert!(approx_eq!(f32, bus0_out[i], 0.4, epsilon = 0.01),
                "bus0[{}]: {} vs 0.4", i, bus0_out[i]);
        
        // Pre-fader send: signal * level = 1.0 * 0.3 = 0.3
        assert!(approx_eq!(f32, bus1_out[i], 0.3, epsilon = 0.01),
                "bus1[{}]: {} vs 0.3", i, bus1_out[i]);
    }
}
...
fn test_mixer_parameters() {
    ...
    // Test set_param
    mixer.set_param("ch_1_volume", ParamValue::Float(0.5)).unwrap();
    if let Some(ParamValue::Float(v)) = mixer.get_param("ch_1_volume") {
        assert!((v - 0.5).abs() < 0.001);
    }
    
    mixer.set_param("ch_2_pan", ParamValue::Float(-0.8)).unwrap();
    if let Some(ParamValue::Float(v)) = mixer.get_param("ch_2_pan") {
        assert!((v + 0.8).abs() < 0.001);
    }
    
    mixer.set_param("ch_1_mute", ParamValue::Bool(true)).unwrap();
