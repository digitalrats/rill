...
use kama_hp::*; // <-- Убираем BufferManagerStats
...
fn test_pooled_buffer_to_hp_conversion() {
    ...
    println!(
        "✅ Converted to HighPrecisionBuffer: {} channels, {} samples, {} Hz",
        hp_buffer.channels(),
        hp_buffer.size(),
        hp_buffer.sample_rate()
    );
...
fn test_hp_processing_with_buffer_manager() {
    ...
    let input_rms: f32 =
        (input_pooled.as_slice().iter().map(|&x| x * x).sum::<f32>() / 512.0).sqrt();
    let output_rms: f32 =
        (output_pooled.as_slice().iter().map(|&x| x * x).sum::<f32>() / 512.0).sqrt();
...
fn test_multichannel_hp_processing() {
    ...
    println!(
        "✅ Multichannel buffer created ({} channels, {} frames)",
        channels, frames
    );
    println!("   Channel frequencies: 440Hz, 880Hz, 1320Hz, 1760Hz");

    // Конвертируем в HighPrecisionBuffer
    let hp_buffer = HighPrecisionBuffer::from_pooled_buffer(&interleaved, channels, 48000.0);
    ...
    for ch in 0..channels {
        ...
        assert!(
            has_positive,
            "Channel {} should have positive values > {}",
            ch, tolerance
        );
        assert!(
            has_negative,
            "Channel {} should have negative values < -{}",
            ch, tolerance
        );
        ...
        assert!(
            (max_val - 0.5).abs() < 0.1,
            "Channel {} max should be near 0.5, got {}",
            ch,
            max_val
        );
        ...
        assert!(
            (min_val + 0.5).abs() < 0.1,
            "Channel {} min should be near -0.5, got {}",
            ch,
            min_val
        );
    ...
    for i in 0..total_samples {
        ...
        if i % 100 == 0 {
            // Печатаем каждый 100-й сэмпл
            println!(
                "  sample {}: original={:.6}, converted={:.6}, diff={:.6}",
    ...
    assert!(
        max_diff < 1e-5,
        "Maximum conversion error too large: {:.6}",
        max_diff
    );
}
...
fn test_precision_comparison() {
    ...
    for i in 0..num_samples.min(1000) {
        // Проверяем первые 1000 сэмплов
        let diff = (output_f64[i] - output_f32[i] as f64).abs();
        error_accum += diff;
        max_error = max_error.max(diff);

        if i < 10 {
            // Печатаем первые 10 для отладки
            println!(
                "  sample {}: f64={:.10}, f32={:.10}, diff={:.10}",
