...
fn test_signal_bus_bounded_drop_newest() {
    ...
    let bus = SignalBus::<TestSignal>::new(
        BusConfig::Bounded(2, OverflowPolicy::DropNewest)
    );
...
fn test_signal_bus_bounded_drop_oldest() {
    ...
    let bus = SignalBus::<TestSignal>::new(
        BusConfig::Bounded(2, OverflowPolicy::DropOldest)
    );
...
fn test_cloned_receivers_get_all_messages() {
    ...
    assert!(got_by_rx1 || got_by_rx2, "At least one receiver should get the message");
    assert!(!(got_by_rx1 && got_by_rx2), "Only one receiver should get the message");
...
fn test_system_event_signal() {
    ...
    bus.send(SystemEvent::Error("test error".to_string())).unwrap();
    let received = receiver.try_recv().unwrap();
    match received {
