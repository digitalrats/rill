...
fn demonstrate_policy(name: &str, policy: OverflowPolicy, values: &[u8]) {
    ...
    let bus = SignalBus::<ControlChange>::new(
        BusConfig::Bounded(3, policy)
    );
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    demonstrate_policy("DropNewest (отбрасывать новые)", 
                      OverflowPolicy::DropNewest, &values);

    // DropOldest - отбрасывает старые сообщения
    demonstrate_policy("DropOldest (отбрасывать старые)", 
                      OverflowPolicy::DropOldest, &values);

    // Создадим отдельный поток для демонстрации Block
    println!("\n--- Демонстрация: Block (блокировка) ---");
    
    let blocking_bus = SignalBus::<ControlChange>::new(
        BusConfig::Bounded(2, OverflowPolicy::Block)
    );
